// Layout
export { Navbar } from './layout/navbar';
export { AppLayout } from './layout/app-layout';

// UI
export { PageHeader } from './ui/page-header';
export { StatCard } from './ui/stat-card';
export { EmptyState } from './ui/empty-state';
export { ConfirmDialog } from './ui/confirm-dialog';
export { FormModal } from './ui/form-modal';
export { PageLoadingOverlay } from './ui/page-loading-overlay';
export type { PageLoadingOverlayProps } from './ui/page-loading-overlay';
export { PdfAberturaBloqueadaModal } from './ui/pdf-abertura-bloqueada-modal';
export type { PdfAberturaBloqueadaModalProps } from './ui/pdf-abertura-bloqueada-modal';
export { Emoji } from './ui/emoji';
export { Tooltip } from './ui/tooltip';
export { EditorRichTextBasico } from './ui/editor-richtext-basico';
export { AssinaturaRelatorio } from './ui/assinatura-relatorio';
export { CrudFiltros } from './ui/crud-filtros';
export { CrudTable } from './ui/crud-table';
export type { CampoFiltro, CrudFiltrosProps } from './ui/crud-filtros';
export type { ColunaTabela, AcaoTabela, CrudTableProps } from './ui/crud-table';

// Relatório
export { GraficoAproveitamentoGrupos } from './relatorio/grafico-aproveitamento-grupos';
export { GraficoDistribuicaoPontos } from './relatorio/grafico-distribuicao-pontos';
export { HistoricoEvolucao } from './relatorio/historico-evolucao';
export { RelatorioTecnicoForm } from './relatorio-tecnico/relatorio-tecnico-form';

// Analytics
export { GoogleAnalytics } from './analytics/google-analytics';

